...
> Scope note (актуализация 2026-04-10): этот документ фиксирует исследовательскую базу фазы. Фактическая реализация в репозитории дополнительно включает операционный контур (docker-compose стенд, mock-сервисы, мониторинг и нагрузочные скрипты), что выходит за минимальный baseline, но не противоречит ему.
